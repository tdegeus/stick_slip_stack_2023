...
public:
    Main(const std::string& fname) : m_file(fname, H5Easy::File::ReadWrite)
    {
        ...
        for (auto& layer : layers) {
            elemmap.push_back(H5Easy::load<xt::xtensor<size_t, 1>>(
                m_file, fmt::format("/layers/{0:d}/elemmap", layer)));

            nodemap.push_back(H5Easy::load<xt::xtensor<size_t, 1>>(
                m_file, fmt::format("/layers/{0:d}/nodemap", layer)));
        }
...
public:
    void run()
    {
        ...
        CheckOrDumpWithDescription(
            m_file,
            "/meta/RunFixedLever/version",
            std::string(MYVERSION),
            "Code version at compile-time.");

        CheckOrDumpWithDescription(
            m_file,
            "/meta/RunFixedLever/version_dependencies",
            deps,
        ...
        if (m_file.exist("/stored")) {
            ...
            this->layerSetTargetUbar(H5Easy::load<xt::xtensor<double, 2>>(
                m_file, fmt::format("/drive/ubar/{0:d}", inc)));
            ...
            H5Easy::dumpAttribute(
                m_file,
                fmt::format("/disp/{0:d}", inc),
                "desc",
                std::string("Displacement (at the end of the increment)."));

            H5Easy::dumpAttribute(
                m_file,
                fmt::format("/drive/ubar/{0:d}", inc),
                "desc",
