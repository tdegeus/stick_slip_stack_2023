...
from GMatElastoPlasticQPot.Cartesian2d import Sigd, Epsd
...
h = np.pi
nlayer = 7
stacking = [False, True, False, True, False, True, False]
...
with h5py.File("myrun_layers=4.h5", "r") as data:
    epsy = data["/epsy"][...]
...
dV = GooseFEM.AsTensor(2, system.dV(), [2, 2])
epsbar = []
sigbar = []

with h5py.File("myrun_layers=4.h5", "r") as data:
    inc = np.sort([int(i) for i in data["disp"]])
    ...
    for i in inc:
        ...
        Eps = system.Eps()
        Sig = system.Sig()
        epsbar += [float(Sigd(np.average(Eps, weights=dV, axis=(0, 1))))]
        sigbar += [float(Sigd(np.average(Sig, weights=dV, axis=(0, 1))))]

epsbar = np.array(epsbar)
sigbar = np.array(sigbar)
...
fig, ax = plt.subplots()
ax.plot(epsbar / 1e-3, sigbar / 2e-3)
...
Sig = system.Sig()
sig = Sigd(np.average(Sig, weights=dV, axis=1))

x = system.coor() + system.u() * 10
gplt.patch(coor=x, conn=system.conn(), lw=0, cindex=sig)
fig.savefig(basename + '_geometry.pdf')
plt.close(fig)
