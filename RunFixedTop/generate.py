...
def generate(myversion, filename, nplates, seed, rid):
    ...
    with h5py.File(filename, "w") as data:
        ...
        mysave(data, "/coor", coor, desc="Nodal coordinates [nnode, ndim]")

        mysave(data, "/conn", conn, desc="Connectivity (Quad4: nne = 4) [nelem, nne]")
        ...
        mysave(data, "/iip", iip, desc="Prescribed DOFs [nnp]")

        mysave(data, "/run/epsd/kick", eps0 * 1e-4, desc="Strain kick to apply")

        mysave(data, "/run/dt", dt, desc="Time step")

        mysave(data, "/rho", rho * np.ones(nelem), desc="Mass density [nelem]")
        ...
        mysave(data, "/cusp/epsy/k", k, desc="Shape factor of Weibull distribution")
        ...
        mysave(data, "/cusp/epsy/nchunk", nchunk, desc="Chunk size")

        mysave(data, "/elastic/elem", elastic, desc="Elastic elements [nelem - N]")
        ...
        mysave(data, "/meta/normalisation/l", h, desc="Elementary block size")

        mysave(data, "/meta/normalisation/rho", rho, desc="Elementary density")

        mysave(data, "/meta/normalisation/G", G, desc="Uniform shear modulus == 2 mu")

        mysave(data, "/meta/normalisation/K", K, desc="Uniform bulk modulus == kappa")

        mysave(data, "/meta/normalisation/eps", eps0, desc="Typical yield strain")

        mysave(data, "/meta/normalisation/sig", 2.0 * G * eps0, desc="== 2 G eps0")

        mysave(data, "/meta/seed_base", seed, desc="Basic seed == 'unique' identifier")
